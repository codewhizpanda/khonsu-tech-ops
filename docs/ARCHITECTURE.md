...
Never cleared by day-close (unlike `saleRows`) — it accumulates until Admin reconciles it. Any logged-in user (Sam/Joyce/Admin) can view the log, add Bisen entries, and edit/delete any entry (auto or manual) to correct mistakes; only Admin can mark an entry credited/pending or force a full resync (see `usePaymentLogs.js` and `PaymentLogsPage.vue`).
