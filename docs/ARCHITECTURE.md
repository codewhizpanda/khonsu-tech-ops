...
Never cleared by day-close (unlike `saleRows`) — it accumulates until Admin reconciles it. Any logged-in user (Sam/Joyce/Admin) can view the log and add Bisen entries; only Admin can mark an entry credited/pending or delete a manual entry (see `usePaymentLogs.js` and `PaymentLogsPage.vue`).
...
`init`, `logSale`, `updateInventoryItems`, `updateUnitStatus`, `savePO`, `updatePOStatus`, `saveProducts`, `saveInventory`, `savePromotions`, `saveFreebies`, `saveUnits`, `saveSettings`, `verifyPin`, `setPin`, `logPayment`, `updatePaymentStatus`, plus GETs `getAllData`, `getSales`, `getPaymentLogs`, `getMasterList` (legacy), `ping`.

`logPayment` and `updatePaymentStatus` (and the `getPaymentLogs` GET) are the exception to the gap below — they **are** implemented in the reference Apps Script in `SetupPage.vue`, against a `Payment Logs` sheet created by `initSheets()`, so payment-log reconciliation works cross-device out of the box for anyone following the Setup wizard.

### ⚠️ Known integration gap
The **Apps Script source embedded in `SetupPage.vue`** (what a store owner actually copies into `script.google.com`) only implements: `doPost` → `init, logSale, logPO, pushInventory, pushMasterList, logPayment, updatePaymentStatus`; `doGet` → `ping, getMasterList, getSales, getPaymentLogs`. It does **not** implement `getAllData`, `verifyPin`, `setPin`, `updateInventoryItems`, `savePO`, `updatePOStatus`, `saveProducts`, `saveInventory`, `savePromotions`, `saveFreebies`, `saveUnits`, or `updateUnitStatus` — all of which the Vue app calls. In practice, for anyone following the in-app Setup wizard today:
